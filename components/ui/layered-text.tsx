...
/**
 * Hover-driven layered text reveal (original component).
 *
 * Each line is an isometric/skewed pair (top + bottom). On `mouseenter`,
 * a GSAP timeline shifts the lines up so the bottom words reveal in a
 * cascading wave; `mouseleave` reverses it.
 *
 * For scroll-driven sync (e.g. linked to a video expansion), see
 * `components/common/ScrollLayeredText.tsx` instead.
 */
export function LayeredText({
  ...
  return (
    <div
      ref={containerRef}
      className={`mx-auto py-24 [font-family:var(--font-figtree)] font-black tracking-[-2px] uppercase text-black dark:text-white antialiased cursor-pointer ${className}`}
